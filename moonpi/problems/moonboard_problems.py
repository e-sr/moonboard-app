...
HOLDS_CONF["grid_name"]["xy"] = [h + str(v) for v in HOLDS_CONF["grid_name"]["vertical"] for h in
                                 HOLDS_CONF["grid_name"]["horizontal"]]


def _get_hold_setup_key(setup):
    l = [k for k, v in list(HOLDS_CONF["setup"].items()) if set(setup) == v]
    if len(l) == 1:
        return l[0]
    elif len(l) > 1:
...
def get_setups(problems):
    setups = set()
    for k, p in list(problems.items()):
        if p["holds_setup_k"]:
...
def problems_data(hold_setup_key, problems):
    ...
    for k, v in list(problems.items()):
        if set(v["holds_setup"]).issubset(current_hold_setup):
            d = {'id': k}
            d.update(v)
            d["holds_setup_short"] = [HOLDS_CONF["configurations"].get(name, {}).get('shortName') for name in
                                      sorted(d["holds_setup"])]
            d['favorite'] = None
            data.append(d)
...
def _new_site_problems_ids_and_author(current_problems={}):
    ...
    current_problem_site_ids = [p['site_id'] for p in list(current_problems.values()) if p['site_id'] is not None]
    new_site_problems_ids = set(all_site_problems_ids.keys()) - set(current_problem_site_ids)
    return {k: v for k, v in list(all_site_problems_ids.items()) if k in new_site_problems_ids}


def _holds_from_site(site_holds):
    holds_d = {}
    for holdType in ['SH', 'IH', 'FH']:
        sortedKeys = sorted([k for k in list(site_holds.keys()) if holdType in k])
...
def fetch_and_save_new_site_problems(current_problems, save_dir_path, log_func, nmax=10):
    ...
    for k, p in list(new_problems_ids.items()):
        try:
        ...
        if n > nmax:
            ...
            break
        i += 1
        log_func("{}/{}, {}%".format(n, n_p, int(old_div(n, n_p * 100))))
